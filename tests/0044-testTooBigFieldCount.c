...
struct TestV10Template {
  ...
  const uint8_t templateBuffer[188];
};

struct TestV10Flow {
  IPFIXFlowHeader flowHeader;
  IPFIXSet flowSetHeader;
  const uint8_t buffer1[161];
    ...
    .flowHeader =
        {
            ...
            /*uint16_t*/.len = 0xd400,     /* The number of records in PDU. */
            /*uint32_t*/.sysUptime =
    ...
    .flowSetHeader =
        {
            ...
            /*uint16_t*/.set_len = 0xc400,
        },

    .templateHeader =
        {
            /*uint16_t*/.templateId = 0x0d01, /*269*/
            /*uint16_t*/.fieldCount = 0x1f00,
        },
...
static const struct TestV10Flow v10Flow = {
    .flowHeader =
        {
            /*uint16_t*/.version = 0x0a00, /* Current version=9*/
            /*uint16_t*/.len = 0xb500,     /* The number of records in PDU. */
            /*uint32_t*/.sysUptime =
                0xdd5d6952, /* Current time in msecs since router booted */
            /*uint32_t*/.flow_sequence =
                0x38040000, /* Sequence number of total flows seen */
...
int main() {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(testFlow, prepare_test_nf10, check_flow),
  };
