...
static void ipv6_and(uint8_t dst[16], const uint8_t s1[16],
                     const uint8_t s2[16]) {
 ...
 * @return       Found node
 */
static struct id_name_description_assoc *
find_id_description_assoc(rd_avl_t *avl, uint64_t id, uint64_t magic) {
 ...
 */
static bool id_name_description_update_string(rd_memctx_t *memctx, char **old,
                                              const char *new,
                                              const size_t new_len,
                                              const char *memory_error_msg,
                                              const char *memory_error_field) {
  assert(old);
...
static void add_id_name_description(rd_avl_t *avl, rd_memctx_t *memctx,
                                    ...
                                    size_t name_len, const char *description,
                                    size_t description_len,
                                    const char *memory_error_msg,
                                    uint64_t magic) {
  assert(avl);

  struct id_name_description_assoc *current =
      find_id_description_assoc(avl, id, magic);
  ...
  const bool name_rc = id_name_description_update_string(
      memctx, &current->name, name, name_len, memory_error_msg, "name");
  if (unlikely(!name_rc)) {
    goto err;
  }

  const bool description_rc = id_name_description_update_string(
      memctx, &current->description, description, description_len,
      memory_error_msg, "description");
...
void observation_id_add_application_id(observation_id_t *observation_id,
  ...
  add_id_name_description(&observation_id->applications,
                          &observation_id->memctx, application_id,
                          application_name, application_name_len, NULL, 0,
...
void observation_id_add_selector_id(observation_id_t *observation_id,
  ...
  add_id_name_description(&observation_id->selectors, &observation_id->memctx,
                          selector_id, selector_name, selector_name_len, NULL,
                          0, "SELECTOR_ID", SELECTOR_ID_MAGIC);
}
...
void observation_id_add_interface(observation_id_t *observation_id,
  ...
  add_id_name_description(&observation_id->interfaces, &observation_id->memctx,
                          interface_id, interface_name, interface_name_len,
...
static const char *id_name_description_assoc_name(rd_avl_t *avl, uint64_t id,
                                                  uint64_t magic) {
  struct id_name_description_assoc *assoc =
      find_id_description_assoc(avl, id, magic);
...
const char *observation_id_selector_name(observation_id_t *observation_id,
                                         uint64_t selector_id) {
  return id_name_description_assoc_name(&observation_id->selectors, selector_id,
                                        SELECTOR_ID_MAGIC);
}
...
observation_id_interface_description(observation_id_t *observation_id,
                                     uint64_t interface_id) {
  return id_name_description_assoc_description(
      &observation_id->interfaces, interface_id, INTERFACE_ID_MAGIC);
}
...
static bool observation_id_add_home_net(observation_id_t *observation_id,
  ...
  struct network_tree_node *home_net =
      rd_memctx_calloc(&observation_id->memctx, 1, sizeof(*home_net));
  if (NULL == home_net) {
    traceEvent(
        TRACE_ERROR,
  ...
  const bool parseAddressrc =
      safe_parse_address(network, &home_net->netAddress);
  if (!parseAddressrc) {
    traceEvent(TRACE_ERROR,
...
static bool parse_observation_id_enrichment(observation_id_t *observation_id,
  ...
  if (!json_is_object(enrichment)) {
    ...
    if (tmp_enrichment) {
      ...
      observation_id->enrichment =
          rd_memctx_strdup(&observation_id->memctx, &tmp_enrichment[1]);
  ...
  if (NULL != dns_ptr_value) {
    if (!json_is_boolean(dns_ptr_value)) {
      traceEvent(TRACE_ERROR, "%s is not a boolean in sensor %s observation id"
                 ...
                 key, sensor_name, observation_id_num(observation_id));
      return false;
...
static observation_id_t *observation_id_new(uint32_t observation_id,
  ...
  const bool parse_oid_rc =
      parse_observation_id(ret, jobservation_id, observation_id, sensor);
  if (!parse_oid_rc) {
...
worker_t *sensor_worker(const struct sensor *sensor) { return sensor->worker; }
...
static const struct network_tree_node *
network_node(observation_id_t *observation_id, const uint8_t ip[16]) {
  assert(observation_id);
  int i;
...
static struct sensor *parse_sensor(json_t *jsensor, const char *ip_str) {
  ...
  json_object_foreach(observations_id, observation_id_key, observation_id) {
    ...
    cur_observation_id =
        observation_id_new(observation_id_n, observation_id, sensor);
...
static struct rb_sensors_db *allocate_rb_sensors_db() {
  ...
  rd_avl_init(&database->sensors.avl, compare_sensors, RD_AVL_F_LOCKS);
  rd_avl_init(&database->bad_sensors.avl, compare_bad_sensors, RD_AVL_F_LOCKS);
...
static bool read_rb_config_sensors_networks(struct rb_sensors_db *database,
  ...
  json_object_foreach(sensors_networks, network, network_config) {
    ...
    if (old_sensor) {
      traceEvent(TRACE_ERROR, "Error: Network %s match with network %s"
                              ". Discarding old one.",
...
static FlowSetV9Ipfix *
find_observation_id_template0(const observation_id_t *observation_id,
...
void save_template(observation_id_t *observation_id,
  ...
  struct flowSetV9Ipfix *prev_template =
      find_observation_id_template0(observation_id, template_id);
  const bool replace_template =
      !prev_template || !template_equal(prev_template, template);

  if (unlikely(readOnlyGlobals.enable_debug)) {
    char buf[BUFSIZ];
    const uint32_t netflow_device_ip = templateInfo->netflow_device_ip;

    traceEvent(TRACE_INFO, "%s [sensor=%s][observation_id=%" PRIu32 "][id=%d]",
               replace_template
                   ? ">>>>> Redefined existing template "
  ...
  if (!template->templateInfo.is_option_template) {
    uint16_t fieldId;
    for (fieldId = 0; fieldId < new_template->templateInfo.fieldCount;
         ++fieldId) {
  ...
  if (templateInfo->templateId < 512) {
    ...
    LIST_INSERT_HEAD(&observation_id->over_512_templates, new_template, entry);
  }
...
void save_template_async(struct sensor *sensor,
  ...
  observation_id_t *observation_id = get_sensor_observation_id(
      sensor, template->templateInfo.observation_domain_id);
  ...
  if (observation_id) {
    ...
    traceEvent(
        TRACE_ERROR,
        "Couldn't async save template %" PRIu16 " of sensor "
...
struct sensor *get_sensor(struct rb_sensors_db *database, uint64_t ip) {
  ...
  struct sensor *found_sensor =
      RD_AVL_FIND_NODE_NL(&database->sensors.avl, &dummy);
  if (found_sensor) {
#ifdef SENSOR_MAGIC
...
observation_id_t *get_sensor_observation_id(struct sensor *sensor,
  ...
  observation_id_t *ret =
      RD_AVL_FIND_NODE_NL(&sensor->observations_id_db, &dummy);
